...
import copy
import math
import statistics  # Medyan için ekledik
...
# Belirtilen nesil sayısı kadar ilerletme fonksiyonu
...
# Bölgenin desenle farkını hesapla (herhangi bir boyutta desen için)
...
# Izgarada belirli desenleri ara (tam eşleşme zorunlu, herhangi bir boyutta desen için)
def find_pattern(grid, patterns):
    ...
    # Desen ızgaradan büyükse hata verme (bu zaten döngüde kontrol ediliyor)
...
# Hedef desen (4x4 Block Cluster - daha zor bir desen)
target_patterns = [
    [[1, 1, 0, 0], [1, 1, 0, 0], [0, 0, 1, 1], [0, 0, 1, 1]]  # 4x4 Block Cluster
]
...
for name, algo in algorithms.items():
    for size in [(5, 5), (10, 10)]:
        ...
        try:
            ...
            print(f"{name} ({rows}x{cols}): Hata: {e}")
            results[f"{name} ({rows}x{cols})"] = (None, None, None)
...
for name, (solution, duration, score) in results.items():
    if solution is not None:
        ...
        print(f"{name}: Çalışmadı (Desen boyutu ızgarayı aşıyor)")
