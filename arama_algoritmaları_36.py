...
# Deneme çalıştırma fonksiyonu (paralel için)
def run_trial(args):
    algo, target_pattern, rows, cols, trial_num, pattern_name, algo_name, key = args
    logging.info(f"{pattern_name} - {algo_name} ({key}): Deneme {trial_num + 1}/100 başladı")
    ...
    if result[1]:
        logging.info(f"{pattern_name} - {algo_name} ({key}): Deneme {trial_num + 1}/100 başarılı")
    else:
        logging.info(f"{pattern_name} - {algo_name} ({key}): Deneme {trial_num + 1}/100 başarısız")
    logging.info(f"{pattern_name} - {algo_name} ({key}): Deneme {trial_num + 1}/100 tamamlandı, süre: {duration:.4f}s")
    return result
...
for pattern_name, pattern in patterns.items():
    for name, algo in algorithms.items():
        for size in [(5, 5), (10, 10), (20, 20)]:
            rows, cols = size
            solution, duration = algo(pattern, rows=rows, cols=cols)
            evolved = evolve_grid(solution)
            score = find_pattern(evolved, pattern)
            results[f"{pattern_name} - {name} ({rows}x{cols})"] = (solution, duration, score)
            
            print(f"{pattern_name} - {name} ({rows}x{cols}):")
    ...
    for pattern_name, pattern in patterns.items():
        for name, algo in algorithms.items():
            for size in [(5, 5), (10, 10), (20, 20)]:
                rows, cols = size
                key = f"{rows}x{cols}"
                trials = [(algo, pattern, rows, cols, i, pattern_name, name, key) for i in range(NUM_TRIALS)]
