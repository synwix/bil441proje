...
def next_generation(grid):
    ...
    # Komşu sayısını vektörize hesaplama
    for di in [-1, 0, 1]:
        for dj in [-1, 0, 1]:
            if di == 0 and dj == 0:
                continue
...
def genetic_algorithm(target_patterns, rows=5, cols=5, pop_size=200, max_generations=500):
    ...
    for gen in range(max_generations):
        scores = [evaluate_individual((ind, target_patterns)) for ind in population]
...
def local_beam_search(target_patterns, rows=5, cols=5, beam_width=10, max_iterations=100):
    ...
    for _ in range(max_iterations):
        scores = [evaluate_individual((beam, target_patterns)) for beam in beams]
        ...
        for beam in beams:
            ...
            for neighbor in neighbors:
                neighbor_score = evaluate_individual((neighbor, target_patterns))
                candidates.append((neighbor_score[0], neighbor_score[1]))
    ...
    with Pool(cpu_count()) as pool:
        for name, algo in algorithms.items():
            for size in [(10, 10), (20, 20)]:
                ...
                results = pool.map(run_trial, trials)
