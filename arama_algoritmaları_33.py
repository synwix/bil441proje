    ...
    for name, algo in algorithms.items():
        for size in [(5, 5), (10, 10), (20, 20)]:
            ...
            for i in range(NUM_TRIALS):
                trial_count += 1
                logging.info(f"{pattern_name} - {name} ({key}): Deneme {trial_count}/{NUM_TRIALS} başladı")
                solution, duration = algo(pattern, rows=rows, cols=cols)
                evolved = evolve_grid(solution)
                score = find_pattern(evolved, pattern)
                ...
                if score == max_score:
                    ...
                    logging.info(f"{pattern_name} - {name} ({key}): Deneme {trial_count}/{NUM_TRIALS} başarılı")
                else:
                    trial_results[pattern_name][name][key]["failure"] += 1
                    logging.info(f"{pattern_name} - {name} ({key}): Deneme {trial_count}/{NUM_TRIALS} başarısız")
                logging.info(f"{pattern_name} - {name} ({key}): Deneme {trial_count}/{NUM_TRIALS} tamamlandı, süre: {duration:.4f}s")
