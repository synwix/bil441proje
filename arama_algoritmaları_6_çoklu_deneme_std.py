...
# Izgarada yalnızca belirli desenleri ara (tam eşleşme zorunlu)
        ...
        for j in range(cols - pattern_cols + 1):
            ...
            for pattern in patterns:
                if all(region[r][c] == pattern[r][c] for r in range(pattern_rows) 
                       for c in range(pattern_cols)):  # Tam eşleşme kontrolü
                    return max_score  # Yalnızca tam eşleşme için maksimum puan
...
# 50 deneme testi (yorum satırlarıyla açıp kapatılabilir)
# Her algoritma ve her boyut için ayrı ayrı 50 deneme yapacak, varyans ve standart sapma eklendi
NUM_TRIALS = 50
trial_results = {
    name: {
...
for name, algo in algorithms.items():
    for size in [(5, 5), (10, 10)]:
        ...
        for _ in range(NUM_TRIALS):
            ...
            score = find_pattern(evolved, target_patterns)
            max_score = len(target_patterns[0]) * len(target_patterns[0][0])
            if score == max_score:  # Tam eşleşme bulundu
                trial_results[name][key]["times"].append(duration)
                trial_results[name][key]["success"] += 1
            else:
                trial_results[name][key]["failure"] += 1

# Sonuçları raporla (varyans ve standart sapma ile)
print("\n50 Deneme İstatistikleri:")
for name in algorithms:
    for size in [(5, 5), (10, 10)]:
        rows, cols = size
        key = f"{rows}x{cols}"
        ...
        # Varyans ve standart sapma hesapla
