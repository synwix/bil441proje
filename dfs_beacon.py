...
def create_grid(size=50):
    ...
    attempts = 0
    while has_beacon(grid) and attempts < 10:
        grid[30:45, 30:45] = np.random.choice([0, 1], size=(15, 15), p=[0.7, 0.3])
        attempts += 1
    return grid
...
def has_beacon(grid):
    beacon1 = np.array([[1, 1], [1, 0]])
    beacon2 = np.array([[1, 1], [0, 1]])
    size = grid.shape[0]
    for i in range(size - 1):
        for j in range(size - 1):
            if np.array_equal(grid[i:i+2, j:j+2], beacon1) or np.array_equal(grid[i:i+2, j:j+2], beacon2):
                return True
    return False
...
if __name__ == "__main__":
    grid = create_grid()
    found, depth, duration, visited_count = dfs_search(grid, has_beacon)
    print("Sonuç:")
    print(f"Beacon bulundu mu? {found}")
